...
DECLARE_JAVA_METHOD(AndroidThunkJava_AndroidAPI_DoesPhoneHaveAccel);
...
void UAndroidAPITemplateFunctions::InitJavaFunctions()
{
	...
	INIT_JAVA_METHOD(AndroidThunkJava_AndroidAPI_DoesPhoneHaveAccel, "([Z)V");
}
...
bool UAndroidAPITemplateFunctions::AndroidAPITemplate_DoesPhoneHaveAccel()
{
#if PLATFORM_ANDROID
	if (JNIEnv* Env = FAndroidApplication::GetJavaEnv(true))
	{
		jbooleanArray Result = Env->NewBooleanArray(1);
		FJavaWrapper::CallVoidMethod(Env, FJavaWrapper::GameActivityThis, AndroidThunkJava_AndroidAPI_DoesPhoneHaveAccel, Result);
